...
var url = require('url');
var POJOStore = require('./POJOStore');
...
function PeerServer(options) {
  ...
  // Connected clients
  this._clients = new this._options.store('clients');

  // Messages waiting for another peer.
  this._outstanding = new this._options.store('outstanding');
  ...
  this._ips = new this._options.store('ips');
...
PeerServer.prototype._initializeWSS = function() {
  ...
  this._wss.on('connection', function(socket) {
    ...
    if (this._clients.has(key) && this._clients.get(key)[id]) {
      self._configureWS(socket, key, id, token);
      return;
    }

    self._checkKey(key, ip, function(err) {
      var keySpace = self._clients.get(key);
      if (err) {
      ...
      if (!keySpace[id]) {
        keySpace[id] = { token: token, ip: ip };
        self._clients.set(key, keySpace);
        self._ips.inc(ip);
...
PeerServer.prototype._configureWS = function(socket, key, id, token) {
  var self = this;
  var client = this._clients.get(key)[id];
...
PeerServer.prototype._checkKey = function(key, ip, cb) {
  if (key === this._options.key) {
    if (!this._clients.has(key)) {
      this._clients.set(key, {});
    }
    if (!this._outstanding.has(key)) {
      this._outstanding.set(key, {});
    }
    if (!this._ips.has(ip)) {
      this._ips.set(ip, 0);
    }
    // Check concurrent limit
    if (Object.keys(this._clients.get(key)).length >= this._options.concurrent_limit) {
      cb('Server has reached its concurrent user limit');
      return;
    }
    if (this._ips.get(ip) >= this._options.ip_limit) {
...
PeerServer.prototype._initializeHTTP = function() {
  ...
  this._app.post(this._options.path + ':key/:id/:token/id', function(req, res, next) {
    ...
    if (self._clients.has(key) && self._clients.get(key)[id]) {
  ...
  this._app.get(this._options.path + ':key/peers', function(req, res, next) {
    var key = req.params.key;
    if (self._clients.has(key)) {
      self._checkAllowsDiscovery(key, function(isAllowed) {
        if (isAllowed) {
          res.send(Object.keys(self._clients.get(key)));
        } else {
          res.send(401);
        }
        return next();
  ...
  var handle = function(req, res, next) {
    ...
    var client;
    if (!self._clients.has(key) || !(client = self._clients.get(key)[id])) {
      if (req.params.retry) {
        res.send(401);
        return next();
      } else {
...
PeerServer.prototype._pruneOutstanding = function() {
  var keys = this._outstanding.keys();
  for (var k = 0, kk = keys.length; k < kk; k += 1) {
    var key = keys[k];
    var outstanding = this._outstanding.get(key);
    var dsts = Object.keys(outstanding);
    for (var i = 0, ii = dsts.length; i < ii; i += 1) {
      var offers = outstanding[dsts[i]];
      var seen = {};
...
PeerServer.prototype._setCleanupIntervals = function() {
  ...
  setInterval(function() {
    var keys = self._ips.keys();
    for (var i = 0, ii = keys.length; i < ii; i += 1) {
      var key = keys[i];
      if (self._ips.get(key) === 0) {
        self._ips.del(key);
      }
    }
  }, 600000);
...
PeerServer.prototype._processOutstanding = function(key, id) {
  var outstandings = this._outstanding.get(key);
  var offers = outstandings[id];
  if (!offers) {
    return;
  }
  for (var j = 0, jj = offers.length; j < jj; j += 1) {
  ...
  this._outstanding.set(key, outstandings);
};

PeerServer.prototype._removePeer = function(key, id) {
  var keySpace = this._clients.has(key) ? this._clients.get(key) : {};
  if (keySpace[id]) {
    this._ips.dec(keySpace[id].ip);
    delete keySpace[id];
    this._clients.set(key, keySpace);
...
PeerServer.prototype._handleTransmission = function(key, message) {
  ...
  var destination = this._clients.get(key)[dst];
  ...
  if (destination) {
    ...
    if (type !== 'LEAVE' && type !== 'EXPIRE' && dst) {
      ...
      outstanding[dst].push(message);
      this._outstanding.set(key, outstanding);
...
PeerServer.prototype._generateClientId = function(key) {
  ...
  while (!!this._clients.get(key)[clientId]) {
